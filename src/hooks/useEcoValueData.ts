 ...
 */
export const useEcoValueData = (facilityId: string | null, weekInterval: number = 4) => {
    return useQuery({
        queryKey: ['ecoValue', facilityId, weekInterval],
        queryFn: async (): Promise<any> => {
            if (!facilityId) {
                throw new Error('Facility ID is required');
            }

            const response = await fetch(`/api/eco-value?facilityId=${facilityId}`);
